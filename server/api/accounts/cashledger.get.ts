...
export default defineEventHandler(async (event) => {
  ...
  try {
    const result = await accountLedgerRowsForApi(client, { companyId, accountType: 'CASH', accountId: null, from, to })
    return {
      cash: { id: 'CASH', type: 'CASH', name: 'Cash', openingBalance: result.openingBalance },
      from,
      to,
