...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    await deleteAccountLedgerForSource(client, { companyId, sourceType: 'INVESTMENT', sourceId: id })
    const res = await client.query(`DELETE FROM investments WHERE id = $1 AND company_id = $2`, [id, companyId])
