...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    await deleteAccountLedgerForSource(client, { companyId, sourceType: 'OPENING', sourceId: id })
    const res = await client.query(`DELETE FROM bank_accounts WHERE id = $1 AND company_id = $2`, [id, companyId])
