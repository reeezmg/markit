...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    /* =================================================
       PRIMARY BANK INFO + BASE OPENING
    ================================================== */
    ...
    const baseOpening = Number(c?.bank || 0)
    ...
    // MONEY TRANSACTIONS (PRIMARY ONLY)
    {
      const r = await client.query(
        `
        SELECT COALESCE(SUM(
    ...
    // ACCOUNT TRANSFERS (PRIMARY ONLY — STRICT FILTER)
    {
      const r = await client.query(
        `
        SELECT
          COALESCE(SUM(
            CASE
              WHEN to_type = 'BANK' AND to_account_id IS NULL
              THEN amount ELSE 0 END
          ), 0)
          -
          COALESCE(SUM(
            CASE
              WHEN from_type = 'BANK' AND from_account_id IS NULL
              THEN amount ELSE 0 END
          ), 0) AS net
        ...
        WHERE company_id = $1
          AND created_at < $2
          AND (
            (from_type = 'BANK' AND from_account_id IS NULL)
            OR
            (to_type = 'BANK' AND to_account_id IS NULL)
          )
        `,
        [companyId, from]
      )
