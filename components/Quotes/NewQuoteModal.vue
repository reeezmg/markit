...
const props = defineProps<{ editingQuote?: any }>()
...
const clientQueryArgs = computed(() => ({
    where: {
        AND: [
            { companies: { some: { companyId } } },
            { deleted: false },
            clientSearch.value
                ? { name: { contains: clientSearch.value, mode: 'insensitive' } }
                : {},
        ],
    },
    take: 20,
...
const saveQuote = async (status: 'DRAFT' | 'SENT') => {
    ...
    try {
        ...
        if (props.editingQuote?.id) {
            // Update existing quote — delete old items, create new ones
