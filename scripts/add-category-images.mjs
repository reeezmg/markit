...
function makeKey(prefix = 'categories', ext = 'jpg') {
...
async function run() {
  ...
  for (const c of categories) {
    const prompt = `Ecommerce hero photo for category '${c.name}' — a clean product-focused shot, white background, show fabric texture and drape, realistic colours, 1024x1024.`;
    console.log('Generating image for category', c.name);
    const gemini = await tryGenerateWithGemini(prompt);
    const dataUrl = gemini || svgDataUrlForText(c.name);
    const key = makeKey('categories', dataUrl.startsWith('data:image/svg') ? 'svg' : 'jpg');
    const url = await uploadBase64ToR2(dataUrl, key);

    await prisma.category.update({ where: { id: c.id }, data: { image: url, banner: url } });
    console.log('Updated category', c.id, 'image ->', url);
  }
