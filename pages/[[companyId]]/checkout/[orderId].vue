    ...
    <div class="bg-gray-50">
      <div class="mx-auto max-w-2xl pt-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
        <div class="space-y-2 px-4 sm:flex sm:items-baseline sm:justify-between sm:space-y-0 sm:px-0">
          ...
          <p class="text-sm text-gray-600">Order placed <time :datetime="orders?.createdAt" class="font-medium text-gray-900">{{ formatDate(orders?.createdAt) }}</time></p>
        ...
        <div class="mt-6">
          ...
          <div class="space-y-8">
            <div v-for="product in orders?.products" :key="product.product.id" class="border-b border-t border-gray-200 bg-white shadow-sm sm:rounded-lg sm:border">
              <div class="px-4 py-6 sm:px-6 lg:grid lg:grid-cols-12 lg:gap-x-8 lg:p-8">
                <div class="sm:flex lg:col-span-7">
                  <div class="aspect-h-1 aspect-w-1 w-full flex-shrink-0 overflow-hidden rounded-lg sm:aspect-none sm:h-40 sm:w-40">
                    <img  :src="`https://unifeed.s3.ap-south-1.amazonaws.com/${product.product.images[0]}`" alt="product picture" class="h-full w-full object-cover object-center sm:h-full sm:w-full" />
                  </div>
                  ...
                  <div class="mt-6 sm:ml-6 sm:mt-0">
                    ...
                    <p class="mt-2 text-sm font-medium text-gray-900">${{ product.product.price }}</p>
                    <p class="mt-3 text-sm text-gray-500">{{ product.product.description }}</p>
                  </div>
                </div>
                  ...
                  <dl class="grid grid-cols-2 gap-x-6 text-sm">
                    <div>
                      ...
                      <dd class="mt-3 text-gray-500">
                        <span class="block">{{ orders?.address.street }}</span>
                        <span class="block">{{ orders?.address.townCity }}</span>
                        <span class="block">{{ orders?.address.postcode }}</span> 
                        <span class="block">{{ orders?.address.country }}</span> 
                      </dd>
                    </div>
                  </dl>
                </div>
              ...
              <div class="border-t border-gray-200 px-4 py-6 sm:px-6 lg:p-8">
                ...
                <div class="mt-6" aria-hidden="true">
                  <div class="overflow-hidden rounded-full bg-gray-200">
                    <div class="h-2 rounded-full bg-indigo-600" :style="{ width: `calc((${steps(product.deliveryStatus)} * 2 + 1) / 8 * 100%)` }" />
                  </div>
                  <div class="mt-6 hidden grid-cols-4 text-sm font-medium text-gray-600 sm:grid">
                    <div class="text-indigo-600">Order Placed</div>
                    <div :class="[steps(product.deliveryStatus) > 0 ? 'text-indigo-600' : '', 'text-center']">Processing</div>
                    <div :class="[steps(product.deliveryStatus) > 1 ? 'text-indigo-600' : '', 'text-center']">Shipped</div>
                    <div :class="[steps(product.deliveryStatus) > 2 ? 'text-indigo-600' : '', 'text-right']">Delivered</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="bg-gray-100 px-4 py-6 sm:rounded-lg sm:px-6 lg:grid lg:grid-cols-12 lg:gap-x-8 lg:px-8 lg:py-8">
            ...
            <dl class="mt-8 divide-y divide-gray-200 text-sm lg:col-span-5 lg:mt-0">
              ...
              <div class="flex items-center justify-between pt-4">
                ...
                <dd class="font-medium text-indigo-600">{{ orders?.total }}</dd>
              </div>
            </dl>
          </div>
        </div>
