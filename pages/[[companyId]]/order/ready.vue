...
try{
  const res = await UpdateBill.mutateAsync({
    where: { id: orderId },
    data: {
      status: allOutOfStock ? 'OUTOFSTOCK' : 'BOOKED'
    }
  });
...
<template>
  <UDashboardPanelContent class="p-1">
        ...
        <template #footer>
          <div
        v-if="!token"
        class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 text-sm mt-4"
      >
        <div class="col-start-1 sm:col-start-2 lg:col-start-6 mb-4 p-1 flex justify-end">
          <UButton ref="packref" class="px-6" @click="handlePack">Pack</UButton>
        </div>
      </div>
    </template>
      </UCard>
