...
const displayItems = computed(() =>
  items.value.filter((item) =>
    item?.entryId || item?.barcode || item?.name || item?.category?.length > 0
  )
)
...
const settlementExpenses = computed(() => {
  const commission = Number(bill.value?.commission || 0)
  const deliveryFees = Number(bill.value?.deliveryFees || 0)
  const waitingFee = Number(bill.value?.waitingFee || 0)
  const storeWaitingFee = Number(bill.value?.storeWaitingFee || 0)
  const storeCancellationFine = Number(bill.value?.storeCancellationFine || 0)
  ...
  return {
    ...
    waitingFee,
    storeWaitingFee,
    storeCancellationFine,
    total: commission + deliveryFees + waitingFee + storeWaitingFee + storeCancellationFine,
  }
})

const totalAfterExpense = computed(() =>
  Number(grandTotal.value || 0) - Number(settlementExpenses.value.total || 0)
)

const appliedCouponCode = computed(() =>
  bill.value?.couponUsage?.[0]?.coupon?.code || '-'
)

const totalDiscount = computed(() =>
  Math.max(0, Number(subtotal.value || 0) - Number(grandTotal.value || 0))
)
...
const couponModel = computed({
  get() {
    return selectedCouponOnbill.value ?? selectedCouponId.value
  },
  set(val) {
    selectedCouponOnbill.value = val
...
<template>
  ...
  <UDashboardPanelContent class="pb-8">
    ...
    <div v-else-if="!bill" class="py-12 text-center text-sm text-gray-500">
      Bill not found.
    </div>
    ...
    <div v-else class="space-y-6 p-4 sm:p-6">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div class="space-y-1">
          <div class="flex flex-wrap items-center gap-2">
            <UButton
              ...
              @click="router.push('/erp/online')"
            >
              Back
            </UButton>
            <UBadge v-if="bill.isMarkit" color="primary" variant="soft">Online</UBadge>
            <UBadge color="gray" variant="soft">{{ paymentMethod || '-' }}</UBadge>
          </div>
          <h1 class="text-xl font-semibold text-gray-900 dark:text-white">
            Invoice #{{ bill.invoiceNumber || '-' }}
          </h1>
          <p class="text-sm text-gray-500">
            Order #{{ bill.orderNumber || '-' }} | {{ new Date(bill.createdAt).toLocaleString('en-IN') }}
          </p>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3 sm:min-w-[280px]">
          <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
            ...
            <div class="mt-1 text-lg font-semibold text-gray-900 dark:text-white">{{ tQty }}</div>
          </div>
          <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
            <div class="text-xs text-gray-500">Grand Total</div>
            <div class="mt-1 text-lg font-semibold text-green-600">{{ formatCurrency(grandTotal) }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="grid gap-4 lg:grid-cols-[1.4fr_1fr]">
        ...
        <UCard>
          <template #header>
            <div class="text-sm font-medium">Bill Summary</div>
          </template>
          ...
          <div class="space-y-3 text-sm">
            ...
            <div class="flex items-center justify-between">
              ...
              <span class="font-medium text-gray-900 dark:text-white">{{ formatCurrency(totalDiscount) }}</span>
            </div>
            <div class="flex items-center justify-between">
              <span class="text-gray-500">Redeemed Points</span>
              <span class="font-medium text-gray-900 dark:text-white">{{ bill?.redeemedPoints || 0 }}</span>
            </div>
            <div class="flex items-center justify-between">
              <span class="text-gray-500">Coupon</span>
              <span class="font-medium text-gray-900 dark:text-white">
                {{ appliedCouponCode }} - {{ formatCurrency(couponValue) }}
              </span>
            </div>
            <div class="flex items-center justify-between">
              <span class="text-gray-500">Bill Points</span>
              <span class="font-medium text-gray-900 dark:text-white">{{ bill?.billPoints || 0 }}</span>
            </div>
            <div class="border-t border-gray-200 pt-3 dark:border-gray-800">
              <div class="flex items-center justify-between">
                ...
                <span class="text-lg font-semibold text-green-600">{{ formatCurrency(grandTotal) }}</span>
              </div>
            </div>
          </div>
        </UCard>
      ...
      <UCard>
        <template #header>
          <div class="text-sm font-medium">Settlement</div>
        </template>

        <div class="space-y-3 text-sm">
          <div class="flex items-center justify-between">
            <span class="text-gray-500">Grand Total</span>
            <span class="font-medium text-gray-900 dark:text-white">{{ formatCurrency(grandTotal) }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-gray-500">Markit Commission</span>
            <span class="font-medium text-red-600">- {{ formatCurrency(settlementExpenses.commission) }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-gray-500">Delivery Fee Share</span>
            <span class="font-medium text-red-600">- {{ formatCurrency(settlementExpenses.deliveryFees) }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-gray-500">Waiting Fee Share</span>
            <span class="font-medium text-red-600">- {{ formatCurrency(settlementExpenses.waitingFee) }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-gray-500">Store Waiting Fee</span>
            <span class="font-medium text-red-600">- {{ formatCurrency(settlementExpenses.storeWaitingFee) }}</span>
          </div>
          <div v-if="settlementExpenses.storeCancellationFine" class="flex items-center justify-between">
            <span class="text-gray-500">Cancellation Fine</span>
            <span class="font-medium text-red-600">- {{ formatCurrency(settlementExpenses.storeCancellationFine) }}</span>
          </div>
          <div class="border-t border-gray-200 pt-3 dark:border-gray-800">
            <div class="flex items-center justify-between">
              <span class="font-medium text-gray-900 dark:text-white">Total After Expense</span>
              <span class="text-lg font-semibold text-primary-600">{{ formatCurrency(totalAfterExpense) }}</span>
            </div>
          </div>
        </div>
      </UCard>

      <UCard v-if="bill.notes">
        <template #header>
      ...
      <UCard>
        <template #header>
          <div class="flex items-center justify-between gap-3">
            ...
            <div class="text-xs text-gray-500">{{ displayItems.length }} lines</div>
          </div>
        </template>
          ...
          <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-800">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-900">
              <tr
                ...
                :key="row.entryId || row.sn"
                :class="row.return ? 'bg-red-50/70 dark:bg-red-950/20' : ''"
              >
                <td class="px-3 py-3 text-gray-900 dark:text-white">{{ row.sn }}</td>
                <td class="px-3 py-3 text-gray-600 dark:text-gray-300">{{ row.barcode || '-' }}</td>
                <td class="px-3 py-3 text-gray-600 dark:text-gray-300">
                  {{ row.category?.map((c) => c.name).join(', ') || '-' }}
                </td>
                <td class="px-3 py-3 text-gray-900 dark:text-white">{{ row.name || '-' }}</td>
                <td class="px-3 py-3 text-right text-gray-900 dark:text-white">{{ row.qty || 0 }}</td>
                <td class="px-3 py-3 text-right text-gray-900 dark:text-white">{{ formatCurrency(row.rate) }}</td>
                <td class="px-3 py-3 text-right text-gray-600 dark:text-gray-300">{{ row.discount || 0 }}</td>
                <td class="px-3 py-3 text-right text-gray-600 dark:text-gray-300">{{ row.tax || 0 }}</td>
      ...
      <UCard v-if="splitPayments?.length">
        <template #header>
          <div class="text-sm font-medium">Split Payments</div>
        </template>
