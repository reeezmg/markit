...
const companyName = computed(() => useAuth().session.value?.companyName || 'Purchase Order')
const companyAddress = computed(() => useAuth().session.value?.address || {})
const companyGstin = computed(() => useAuth().session.value?.gstin || '')
          ...
          select: {
            ...
            variants: {
              select: {
                ...
                sprice: true,
                items: {
                  select: { id: true, size: true, barcode: true, initialQty: true },
                },
              },
            },
          },
...
const getPaidAmount = (po: any) =>
  (po?.distributorPayment ?? []).reduce((sum: number, p: any) => sum + Number(p.amount || 0), 0)
...
const detailsLines = computed(() => purchaseOrderLineRows(selectedDetailsRow.value))
...
<template>
  <UDashboardPanelContent class="p-4">
    ...
    <!-- ─── PO Details Modal ─── -->
    <UModal v-model="isDetailsOpen" :ui="{ width: 'sm:max-w-5xl' }">
      <UCard
        :ui="{
      ...
      >
        ...
        >
          ...
          <div class="grid gap-4 border-b border-gray-200 p-5 dark:border-gray-800 md:grid-cols-3">
            ...
            <div class="space-y-1">
              ...
              <p class="font-semibold">{{ selectedDistributor?.distributor?.name || '-' }}</p>
              <p class="text-sm text-gray-600 dark:text-gray-300">
                {{ formatAddress(selectedDistributor?.distributor?.address) || '-' }}
              </p>
              <p v-if="selectedDistributor?.distributor?.gstin" class="text-sm text-gray-600 dark:text-gray-300">
                GSTIN: {{ selectedDistributor.distributor.gstin }}
              </p>
            </div>
        ...
        <template #footer>
          <div class="po-no-print flex justify-end gap-2">
            <UButton color="gray" variant="soft" label="Close" @click="isDetailsOpen = false" />
            <UButton color="primary" icon="i-heroicons-printer-20-solid" label="Print" @click="printPurchaseOrder()" />
          </div>
        </template>
      </UCard>
    </UModal>
