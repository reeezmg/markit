...
const returnNo   = ref<number | null>(null)
...
onMounted(async () => {
  ...
  try {
    const [catData, pr] = await Promise.all([
      $fetch<any[]>('/api/bill/findManyCategory', { query: { companyId: companyId.value } }),
      $fetch<any>(`/api/purchasereturn/${returnId}`),
    ])
    categories.value = catData ?? []

    returnNo.value              = pr.returnNo
    selectedDistributorId.value = pr.distributorId
    remarks.value               = pr.remarks ?? ''
    returnDate.value            = new Date(pr.createdAt).toISOString().split('T')[0]

    // Match PO option after distData loads
    if (pr.purchaseOrderId) {
      await nextTick()
      const po = (selectedDistComp.value?.purchaseOrders ?? []).find(p => p.id === pr.purchaseOrderId)
      if (po) {
        selectedPO.value = {
          label: `PO#${po.purchaseOrderNo}${po.billNo ? ' / ' + po.billNo : ''} — ₹${Number(po.totalAmount || 0).toFixed(2)}`,
          id: po.id,
        }
      }
    }

    returnRows.value = pr.items.map((item: any) => ({
    ...
    toast.add({ title: 'Failed to load return', description: err.message, color: 'red' })
  } finally {
...
const handleSubmit = async () => {
  ...
  isSaving.value = true
  try {
    await $fetch('/api/purchasereturn/update', {
      method: 'PUT',
      body: {
        id:              returnId,
    ...
    toast.add({ title: 'Purchase return updated', color: 'green' })
    router.push('/distributor/purchase-return')
  } catch (err: any) {
    toast.add({ title: 'Failed to update', description: err.data?.message || err.message, color: 'red' })
  } finally {
  ...
  <UDashboardPanelContent class="p-1 flex flex-col gap-2">
    <div class="flex items-center gap-2 px-1">
      ...
      <span class="text-sm font-semibold text-gray-700 dark:text-gray-200">
        Edit Purchase Return{{ returnNo ? ` #${returnNo}` : '' }}
      </span>
    </div>
