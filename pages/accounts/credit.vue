...
<template>
  <UDashboardPanelContent class="pb-24">
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
      <UCard v-if="account">
        <div class="text-sm text-gray-500">Opening Balance (Credit)</div>
        <div class="text-xl font-semibold">{{ formatCurrency(account.openingBalance) }}</div>
      </UCard>

      <UCard>
        <div class="text-sm text-gray-500">Total Credit In</div>
        <div class="text-xl font-semibold text-green-600">{{ formatCurrency(totalCreditIn) }}</div>
      </UCard>

      <UCard>
        <div class="text-sm text-gray-500">Total Credit Out</div>
        <div class="text-xl font-semibold text-red-600">{{ formatCurrency(totalCreditOut) }}</div>
      </UCard>
    </div>
    ...
    >
      ...
      >
        <template #date-data="{ row }">
          <span v-if="row.source !== 'OPENING'">{{ format(new Date(row.date), 'dd MMM yyyy') }}</span>
          <span v-else class="text-gray-400">-</span>
        </template>

        <template #debit-data="{ row }">
          <span v-if="row.debit > 0" class="text-red-600">{{ formatCurrency(row.debit) }}</span>
          <span v-else>-</span>
        </template>

        <template #credit-data="{ row }">
          <span v-if="row.credit > 0" class="text-green-600">{{ formatCurrency(row.credit) }}</span>
          <span v-else>-</span>
        </template>
        ...
        <template #source-data="{ row }">
          <UBadge size="xs" color="gray">{{ row.source }}</UBadge>
        </template>
      </UTable>
