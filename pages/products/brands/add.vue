...
const handleSubmit = async () => {
  if (!name.value) {
    toast.add({ title: 'Brand name is required', color: 'red' });
    return;
  }
...
<template>
  <UDashboardPanelContent class="pb-24">
    <UPageCard class="m-3">
      <div class="text-xl mb-4">Brand</div>
      <hr class="h-px my-6 bg-gray-200 border-0 dark:bg-gray-700" />

      <div class="flex flex-col gap-6 lg:flex-row lg:items-start">
        <div class="flex flex-col gap-4 shrink-0">
          <BannerUpload label="Logo / Image" variant="square" @update="imageFile = $event" />
        ...
        <div class="w-full space-y-4">
          ...
          <BannerUpload label="Banner (shown on dark-store Brands tab)" variant="wide" @update="bannerFile = $event" />
        </div>
      </div>
